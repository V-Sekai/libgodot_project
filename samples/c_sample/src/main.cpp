...
typedef struct {
    void *handle;
    GDExtensionObjectPtr (*func_gdextension_create_godot_instance)(int, char *[], GDExtensionInitializationFunction);
} LibGodot;
...
LibGodot* libgodot_new(const char* p_path) {
    ...
    self->func_gdextension_create_godot_instance = (GDExtensionObjectPtr (*)(int, char *[], GDExtensionInitializationFunction))dlsym(self->handle, "gdextension_create_godot_instance");
...
int main(int argc, char **argv) {
    ...
    char* args[] = { program, path_arg, project_path, NULL };

    GDExtensionObjectPtr instance = libgodot->func_gdextension_create_godot_instance(sizeof(args)/sizeof(char*) - 1, args, gdextension_default_init);
    if (instance == NULL) {
        fprintf(stderr, "Error creating Godot instance\n");
        return EXIT_FAILURE;
    }

    godot::GodotInstance *godot_instance = reinterpret_cast<godot::GodotInstance *>(godot::internal::get_object_instance_binding(instance));
